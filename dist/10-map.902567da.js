...
var container = d3.select('#chart-10'); // Create your scales

var xPositionScale = d3.scaleLinear().domain([2010, 2019]).range([0, width]);
var yPositionScale = d3.scaleLinear().domain([0, 8000]).range([height, 0]);
var line = d3.line().x(function (d) {
  return xPositionScale(d.Date);
}).y(function (d) {
  return yPositionScale(d.Unique);
});
d3.csv(require('/data/Borough_year.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  container.selectAll('svg').data(nested).enter().append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').each(function (d) {
    ...
    svg.append('path').datum(d.values).attr('d', line).attr('stroke', '#7fbf7b').attr('fill', 'none').attr('stroke-width', 2);
    svg.append('text').text(d.key).attr('x', width / 2).attr('text-anchor', 'middle').attr('fill', '#7fbf7b').attr('y', -15).attr('font-weight', 'bold').attr('font-size', 14); // container
    //   .append('text')
    //   .attr('font-size', '18')
    //   .attr('text-anchor', 'middle')
    //   .text('Complaints Peaked in 2017 in All Boroughs, Except Staten Island')
    ...
    var yAxis = d3.axisLeft(yPositionScale).tickValues([2000, 4000, 6000, 8000]).tickSize(-width);
    svg.append('g').attr('class', 'axis y-axis').call(yAxis);
    svg.selectAll('.y-axis path').remove();
    svg.selectAll('.y-axis line').attr('stroke-dasharray', 2).attr('stroke', 'grey');
  });
}
},{"d3":"../node_modules/d3/index.js","/data/Borough_year.csv":"data/Borough_year.csv"}]},{},["scripts/10-map.js"], null)
//# sourceMappingURL=/10-map.902567da.js.map